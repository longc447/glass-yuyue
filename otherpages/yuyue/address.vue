						...
						<radio-group @change="radioChange">
							<view class="address-item" v-for="(item, index) in addressList" :key="index">
								<view class="address-item-top">
									<view style="display: flex;">
										<!-- <view class="iconfont" style="margin: auto 0;padding-right: 30rpx;" 
									:class="item.is_default == 1 ? 'iconyuan_checked color-base-text' : 'iconyuan_checkbox'"></view>
									 -->
										<view style="margin: auto 0;padding-right: 30rpx;transform:scale(0.7)">
								...
								<view class="address-item-bottom">
									<view class="address-default">
										...
										<text class="default" :class="{ 'color-base-text': item.is_default == 1 }"
											@click="setDefault(item.id)">默认地址</text>
									</view>
							...
							<view class="btn-add">
								...
								<view class="wx-add" @click="getChooseAddress()" v-if="local != 1">
									<text class="">获取地址</text>
								</view>
								<!-- #endif -->
								<!-- #ifdef H5 -->
								<button type="primary" class="add-address" @click="getChooseAddress()"
									v-if="$util.isWeiXin() && local != 1">获取地址</button>
								<!-- #endif -->
								<button type="primary" class="add-address" @click="addAddress('add')"
									style="background-color: #c9c9c9;width: 200rpx;margin: 0 auto;height: 80rpx;">
									<text class="iconfont iconadd1"></text>
									添加地址
								</button>
								<button type="primary" class="add-address" @click="toAdd()">
...
<script>
	...
	export default {
		data() {
			return {
				addressList: [],
				back: '/otherpages/yuyue/address', // 返回页
		...
		methods: {
			toAdd(){
				var sn = this.addressList[this.current]
				this.$util.redirectTo('/otherpages/yuyue/time?sn='+ encodeURIComponent(JSON.stringify(sn)));
			},
