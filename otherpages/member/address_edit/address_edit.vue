		...
		selectAddress() {
			...
			uni.chooseLocation({
				success: res => {
					...
					urlencode.getAddress(res.latitude + ',' + res.longitude);
				},
				fail(res) {
					uni.getSetting({
			...
			uni.getLocation({
			    ...
			    success: function (res) {
					...
					urlencode.formData.latitude = res.latitude;
					urlencode.formData.longitude = res.longitude;
					//this.formData.address = res.name;
					//urlencode.getAddress(28.1 + ',' + 113.0);
					urlencode.getAddress(res.latitude + ',' + res.longitude);
