	...
	<view class='issue'>
		<view class="issue-head">
			...
			<view class="issue-head-star-box" v-if="starsShow">
				<image v-for="(item,index) in starsMax" :key="index" :src="(index+1)>formatScore?starDefault:starActive" :class="formatScore==index+1?'active':''" mode="" @click="setScore(index+1)">{{formatScore}}</image>
			</view>
		</view>
		 <textarea v-if="textareaShow" @blur="blur" :value="infoReceive.textareaValue" :placeholder="textareaPlaceholder" @input="textareaSet"/>
	...
	export default {
		...
		methods: {
			...
			setScore(score){
				...
				console.log(this.infoReceive.score)
				this.$emit("scoreChange",score)
			},
			textareaSet(score){
				console.log(score)
				this.infoReceive.textareaValue=score.detail.value
			},
			/**
			 * @name 获取textarea内容
			 */
